...
class DataLoader(object):
    ...
    def __init__(self, filename, train, valid, device, seq_in, seq_out,label_len,normalize=True):
        self.P = seq_in
        self.H = seq_out
        self.L = label_len or seq_out
        self.rawdat = np.load(filename)
    #    self.rawdat = (self.rawdat[:,:,0])[:,:,None]
    ...
    def _batchify(self, idx_set):
        ...
        X_in = torch.zeros((n,self.m,self.L+self.H,self.d))
        ...
        for i in range(n):
            ...
            X_in[i,:,:self.L,:] = torch.from_numpy(self.dat[:,end-self.L:end, :])
    ...
    def get_batches(self, enc_in ,dec_in, targets, batch_size, shuffle=False):
        length = len(enc_in)
        if shuffle:
        ...
        while (start_idx < length):
            ...
            yield Variable(X),Variable(X_in),Variable(Y),end_idx
